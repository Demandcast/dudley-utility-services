...
- [2026-09-06] `--teal-050` ban extends to component fills (avatar placeholders, photo slots). Swap to `--neutral-100`. The light blue reads as a different palette from the neutral grays used everywhere else. Jenny caught this in brand book PDF review (#98).
- [2026-09-06] **Ink vs teal rule.** Teal field is the default dark surface. Ink field is the alternate — use only when consecutive dark slides need visual distinction (decks, carousels). Single dark sections (heroes, footers, brand book demos) always use teal. The brand book had one motif demo on ink and another on teal on the same page — that inconsistency is what prompted the rule.
